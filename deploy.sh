...
echo "" >> changelog.txt && \
echo "" >> changelog.txt && \
echo "### :package: [Download JAR](https://repo1.maven.org/maven2/com/erudika/para-queue-sqs/${ver}/para-queue-sqs-${ver}-shaded.jar)" >> changelog.txt && \
gh release create -F changelog.txt -t "v$ver" $ver && \
rm changelog.txt

docker build -t erudikaltd/para-queue-sqs:${ver} . && docker push erudikaltd/para-queue-sqs:${ver}
